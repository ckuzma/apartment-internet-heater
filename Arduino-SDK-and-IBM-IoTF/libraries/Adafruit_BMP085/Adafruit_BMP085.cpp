...
boolean Adafruit_BMP085::begin(uint8_t mode) {
  ...
  Wire.begin(13, 12); // Change this to the GPIOs for (SDA, SCL)
...
int32_t Adafruit_BMP085::readSealevelPressure(float altitude_meters) {
  ...
  return (int32_t)(pressure / powr(1.0-altitude_meters/44330, 5.255));
}
...
float Adafruit_BMP085::readAltitude(float sealevelPressure) {
  ...
  altitude = 44330 * (1.0 - powr(pressure /sealevelPressure,0.1903));
